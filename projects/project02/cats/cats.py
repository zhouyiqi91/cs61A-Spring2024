...
def about(subject: list[str]):
    ...
    def func(paragraph: str) -> bool:
        words = set(x.lower() for x in remove_punctuation(paragraph).split())
        return any(x in words for x in subject)
    return func
    # END PROBLEM 2
...
def accuracy(typed: str, source: str) -> float:
    ...
    correct = 0
    for s,t in zip(source.split(), typed.split()):
        correct += int(s==t)
    return correct / len(typed.split()) * 100
    # END PROBLEM 3


def wpm(typed, elapsed) -> float:
...
def autocorrect(
    ...
    from TYPED_WORD. If the typed_word is contained inside the word_list, autocorrect returns that word.If multiple words are tied for the smallest difference,
    ...
    for word in word_list:
        ...
        cur_limit = diff_function(typed_word, word, limit)
        if cur_limit < min_limit:
            min_limit = cur_limit
            res = word
    return res if min_limit <= limit else typed_word
    """
    if typed_word in word_list: return typed_word
    res = min(word_list, key=lambda word: diff_function(typed_word,word,limit))
    min_diff = diff_function(typed_word,res,limit)
    return res if min_diff <= limit else typed_word

    # END PROBLEM 5


def feline_fixes(typed: str, source: str, limit) -> int:
    ...
    # BEGIN PROBLEM 6
    import math
    if limit < 0: return math.inf # type: ignore
    if not typed: return len(source)
    if not source: return len(typed)
    diff = int(typed[0]!=source[0])
...
def minimum_mewtations(typed:str, source:str, limit) -> int:
